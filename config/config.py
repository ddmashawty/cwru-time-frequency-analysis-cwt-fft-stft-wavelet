...
class Config:
    ...
    # ========================================================================
    # 分段窗口大小 (样本数)
    WINDOW_SIZE: int = 4096        # 增大到4096点，容纳10个30Hz周期，消除频谱泄漏
    
    # 连续窗口之间的重叠比例 (0.0 - 1.0)
    OVERLAP_RATIO: float = 0.75      # 提高至75%，补偿窗口增大导致的样本减少
    
    # 窗口之间的步长（固定值）
    STRIDE: int = 1024             # 4096 * 0.25 = 1024
    ...
    # --- 连续小波变换设置 ---
    CWT_DT: float = 1/12000            # 采样周期
    CWT_WAVELET: str = "cmor1.0-2"     # 复Morlet小波 (中心频率参数B=1.0, 带宽参数C=2)
    CWT_SCALES: int = 256              # 尺度数量 (totalscal)
