...
class SignalPreprocessor:
    ...
    def _downsample(self, signal: np.ndarray, original_sr: int) -> np.ndarray:
        ...
        使用 scipy.signal.decimate，它在降采样前应用抗混叠滤波器
        以防止频谱混叠。
        ...
        try:
            # 使用 IIR 滤波器的 decimate 以获得更好的性能
            # zero_phase=True 表示前后都应用滤波器
            downsampled = decimate(signal, factor, ftype='iir', zero_phase=True)
        except Exception:
            # 备选方案: 简单平均 (不太理想但更稳健)
